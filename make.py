...
def main(clean: bool):
    ...
    if clean:
        print("Cleaning...")
        subprocess.run(
            "py -m esptool --chip esp32 -b 460800 --before=default_reset --after=hard_reset erase_flash"
        )
        subprocess.run(
            "py -m esptool --chip esp32 -b 460800 --before=default_reset --after=hard_reset write_flash --flash_mode dio --flash_freq 40m --flash_size 4MB 0x1000 bin/bootloader/bootloader.bin 0x10000 bin/micropython.bin 0x8000 bin/partition_table/partition-table.bin "
        )
        # subprocess.run("py -m mpremote bootloader")
        # subprocess.run("py -m esptool erase_flash")
        # subprocess.run(
        #     "py -m esptool --chip esp32 -b 460800 --before=default_reset --after=hard_reset write_flash --flash_mode dio --flash_freq 80m --flash_size 4MB 0x1000 bin/bootloader/bootloader.bin 0x10000 bin/micropython.bin 0x8000 bin/partition_table/partition-table.bin "
        # )
        time.sleep(2)
    ...
    subprocess.run("py -m mpremote fs cp -r ./fonts : ")
    subprocess.run("py -m mpremote fs cp -r ./lib : ")
